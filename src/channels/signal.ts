...
import { getLatestMessage, getMessageSender } from '../db.js';
...
interface SignalQuote {
  id?: number; // target message's sent timestamp
...
export class SignalChannel implements Channel {
  ...
  async sendReaction(
    ...
    const targetTs = parseInt(messageKey.id.replace('signal-', ''), 10);
    const targetAuthor = messageKey.fromMe
      ? this.opts.accountNumber
      : (getMessageSender(messageKey.id, chatJid) ?? '');
    ...
    if (chatJid.startsWith('signal:group:')) {
      params.groupId = chatJid.slice('signal:group:'.length);
    } else {
      params.recipient = [chatJid.slice('signal:'.length)];
    }

    await this.rpcCall('sendMessageReaction', params);
    logger.debug({ chatJid, emoji }, 'Signal reaction sent');
  }

  async reactToLatestMessage(chatJid: string, emoji: string): Promise<void> {
    const latest = getLatestMessage(chatJid);
    if (!latest) return;
    const ts = parseInt(latest.id.replace('signal-', ''), 10);
    await this.sendReaction(
      chatJid,
      { id: latest.id, timestamp: ts, fromMe: latest.is_from_me === 1 },
      emoji,
    );
  }
  ...
  private handleNotification(notification: JsonRpcNotification): void {
    ...
    if (groups[chatJid]) {
      ...
      this.opts.onMessage(chatJid, {
        ...
        quoted_message_id: quote?.id ? `signal-${quote.id}` : undefined,
        quote_sender_name: quote?.authorName || quote?.author,
